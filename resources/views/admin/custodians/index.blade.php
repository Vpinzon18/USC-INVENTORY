                        ...
                        <tbody class="divide-y divide-gray-100">
                            @forelse ($custodians as $custodian)
                                <tr class="hover:bg-gray-50 transition-colors">
                                    <td class="px-6 py-4 font-medium text-gray-900">{{ $custodian->full_name }}</td>
                                    <td class="px-6 py-4 text-gray-600">{{ $custodian->job_title }}</td>
                                    <td class="px-6 py-4 font-mono text-xs text-gray-500">{{ $custodian->document_number }}</td>
                                    <td class="px-6 py-4">
                                        <span class="px-2 py-1 bg-slate-100 text-slate-700 rounded text-xs font-semibold uppercase">{{ $custodian->dependency }}</span>
                                    </td>
                                    <td class="px-6 py-4 text-gray-600">{{ $custodian->email ?? 'N/A' }}</td>
                                    <td class="px-6 py-4 text-gray-600">{{ $custodian->extension ?? '---' }}</td>
                                    <td class="px-6 py-4 text-center">
                                        <div class="flex justify-center space-x-2">
                                            <a href="{{ route('custodians.edit', $custodian) }}" class="text-blue-600 hover:bg-blue-50 p-2 rounded-lg transition">
                                            ...
                                            <form action="{{ route('custodians.destroy', $custodian) }}" method="POST" class="inline">
                                                @csrf @method('DELETE')
                                                <button type="submit" onclick="return confirm('¿Eliminar responsable?')" class="text-red-600 hover:bg-red-50 p-2 rounded-lg transition">
