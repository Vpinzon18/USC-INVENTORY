    ...
    <div class="py-6 bg-slate-50 min-h-screen font-sans text-slate-800" x-data="buildingsEngine()" x-cloak>
        <div class="max-w-[1400px] w-[96%] mx-auto space-y-6">
            ...
            <div class="bg-white rounded-2xl shadow-sm border border-slate-200 p-6 flex flex-col lg:flex-row justify-between items-start lg:items-center gap-6 relative overflow-hidden">
                <!-- Línea decorativa izquierda en tono Azul -->
                ...
                <div class="flex flex-col sm:flex-row gap-3 w-full lg:w-auto">
                    <a href="{{ route('buildings.create') }}" class="inline-flex justify-center items-center gap-2 bg-blue-600 hover:bg-blue-700 text-white font-bold text-sm px-6 py-2.5 rounded-xl shadow-sm transition-all active:scale-95 uppercase tracking-wider">
                ...
                <div class="flex flex-col lg:flex-row justify-between items-center gap-4">
                    ...
                    <div class="relative w-full lg:w-96 flex-1">
                        ...
                        <input type="text" x-model="search" @input.debounce.500ms="applyFilters()" placeholder="Buscar bloque o edificio..."
                            class="w-full pl-10 pr-3 py-2.5 bg-slate-50 border border-slate-200 rounded-xl text-sm font-medium text-slate-700 focus:bg-white focus:ring-2 focus:ring-blue-500 focus:border-blue-500 transition-all shadow-sm">
                    </div>
                    ...
                    <div class="flex flex-wrap items-center gap-3 w-full lg:w-auto">
                        ...
                        <div class="relative" @click.away="campusOpen = false">
                            <button @click="campusOpen = !campusOpen" class="bg-slate-50 border border-slate-200 text-slate-700 text-sm font-bold py-2.5 px-4 rounded-xl flex items-center gap-2 hover:bg-slate-100 transition-colors w-full sm:w-auto">
                            ...
                            <div x-show="campusOpen" x-transition x-cloak class="absolute right-0 sm:left-0 mt-2 w-full sm:w-72 bg-white border border-slate-200 rounded-xl shadow-xl z-50 overflow-hidden">
                                <div class="p-2 border-b border-slate-100 bg-slate-50 relative">
                                    <input type="text" x-model="campusSearch" @input.debounce.300ms="fetchCampuses()" placeholder="Buscar sede..." class="w-full px-3 py-2 pr-8 text-xs border-slate-200 rounded-lg focus:ring-blue-500 focus:border-blue-500 shadow-sm" @click.stop>
                                ...
                                <ul class="max-h-56 overflow-y-auto py-1 text-sm">
                                    <li @click="setCampus('', ''); campusOpen = false" class="px-4 py-2.5 hover:bg-slate-50 cursor-pointer text-slate-500 font-bold border-b border-slate-100">
                                        Todas las Sedes
                                    </li>
                                    
                                    <!-- Iteración dinámica AJAX -->
                                    <template x-for="campus in campuses" :key="campus.id">
                                        <li @click="setCampus(campus.id, campus.name); campusOpen = false" class="px-4 py-2 hover:bg-slate-50 cursor-pointer text-slate-700 font-medium truncate flex items-center gap-2">
                        ...
                        <button @click="clearFilters()" class="p-2.5 text-slate-400 hover:text-slate-700 hover:bg-slate-100 rounded-xl border border-transparent transition-all" title="Limpiar Filtros">
            ...
            <div id="table-container" class="relative z-10 transition-opacity duration-300">
                <div class="bg-white rounded-2xl shadow-sm border border-slate-200 overflow-hidden flex flex-col">
                                    ...
                                    <tr class="hover:bg-slate-50/50 transition-colors group">
                                        ...
                                        <td class="px-6 py-5 text-center">
                                            <div class="flex items-center justify-center gap-1">
                                                <a href="{{ route('buildings.edit', $building) }}" class="p-2 text-slate-400 hover:text-blue-600 hover:bg-blue-50 rounded-lg transition-colors" title="Editar Bloque">
                                                    <svg class="w-5 h-5" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M11 5H6a2 2 0 00-2 2v11a2 2 0 002 2h11a2 2 0 002-2v-5m-1.414-9.414a2 2 0 112.828 2.828L11.828 15H9v-2.828l8.586-8.586z"></path></svg>
                                                </a>
                                                
                                                <!-- Se mantiene rojo por ser una acción destructiva -->
                                                <form action="{{ route('buildings.destroy', $building) }}" method="POST" class="inline" @submit.prevent="confirmDelete($event, '{{ addslashes($building->name) }}')">
                                                    @csrf 
                                                    @method('DELETE')
                                                    <button type="submit" class="p-2 text-slate-400 hover:text-rose-600 hover:bg-rose-50 rounded-lg transition-colors" title="Eliminar Bloque">
                                    ...
                                    <tr>
                                        <td colspan="4" class="px-6 py-20 text-center">
                                            <div class="flex flex-col items-center justify-center">
                                                ...
                                                <h3 class="text-base font-extrabold text-slate-800 mb-2">No existen bloques registrados</h3>
                                                <p class="text-sm text-slate-500 mb-6 max-w-md mx-auto">Agregue el primer bloque para comenzar la organización de la infraestructura institucional.</p>
                                                <a href="{{ route('buildings.create') }}" class="text-sm font-bold text-blue-600 hover:text-blue-700 transition-colors uppercase tracking-wider flex items-center gap-1.5 bg-blue-50 px-5 py-2.5 rounded-xl border border-blue-100 hover:bg-blue-100">
                                                    <svg class="w-4 h-4" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M12 4v16m8-8H4"/></svg>
                    ...
                    <!-- FOOTER SIMPLE -->
                    <div class="bg-slate-50 border-t border-slate-200 p-5 flex items-center justify-center sm:justify-between">
                        @if(isset($buildings) && method_exists($buildings, 'hasPages') && $buildings->hasPages())
                            <div class="w-full">
                                {{ $buildings->appends(request()->query())->links() }}
                            </div>
                        @else
                            <p class="text-xs text-slate-500 font-bold uppercase tracking-widest w-full text-center">Fin de los registros.</p>
                        @endif
                    </div>
                </div>
            </div>
        ...
        document.addEventListener('alpine:init', () => {
            Alpine.data('buildingsEngine', () => ({
                ...
                async fetchCampuses() {
                    ...
                    try {
                        // 🔴 IMPORTANTE: Cambiamos a la URL correcta basada en tu configuración de API
                ...
                confirmDelete(event, name) {
                    Swal.fire({
                        ...
                        html: `Esta acción eliminará permanentemente el bloque <br><strong class="text-slate-900 text-lg">${name}</strong><br><br>y podría afectar oficinas o salones asociados.`,
                        icon: 'warning',
                        showCancelButton: true,
                        confirmButtonColor: '#2563eb', // Color blue-600
